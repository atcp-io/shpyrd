...
Homebrew tap and a curl installer, a multi-arch `ghcr.io/atcp-io/shpyrd-server` image built
...
- `brew install atcp-io/tap/shpyrd` or `curl -fsSL get.shpyrd.io | sh`, then
...
- GitHub Actions: `ci.yml` (go test, vet, UI lint/build, e2e on kind: create cluster,
  ...
  tags: GoReleaser (CLI archives, checksums, Homebrew formula in `atcp-io/homebrew-tap`),
...
- `pkg/version.Version` from the tag; the local profile's `SHPYRD_SERVER_IMAGE` becomes
  `ghcr.io/atcp-io/shpyrd-server:${SHPYRD_VERSION}`; dev builds keep `--set`.
