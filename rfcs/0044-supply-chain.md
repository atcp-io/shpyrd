# RFC-0044 Supply chain and encryption at rest
...
- `shpyrd cluster status` shows image digests, drift from the pinned set and whether
...
- **Encryption at rest**: `shpyrd cluster create` configures the kind API server with an
  `EncryptionConfiguration` (AES-GCM key generated by the CLI, kept in `~/.shpyrd`) so
  Secrets — config vars, credentials, sessions — are encrypted in etcd; managed clusters
  document the provider setting (EKS envelope encryption with KMS) and `cluster status`
  reports whether encryption is on.
