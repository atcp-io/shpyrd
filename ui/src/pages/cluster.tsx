  ...
  return (
    <div className="grid gap-6">
      <div className="grid gap-4 md:grid-cols-4">
        <Info label="Environment profile" value={c.install?.profile ?? '-'} hint={profileHint(c.install?.profile)} />
        <Info label="shpyrd version" value={c.install?.version ?? '-'} mono />
        <Info label="Domain" value={c.install?.domain ?? '-'} mono hint="Projects are published at <name>.<domain>" />
        <Info label="Base stack updated" value={c.install?.updatedAt ? ago(c.install.updatedAt) : '-'} hint="last shpyrd cluster init" />
      </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div>
            ...
            <CardDescription>
              <strong>Used</strong> is what the machines are doing right now; <strong>reserved</strong> is what running processes have
        ...
        <CardContent className="grid gap-6">
          {metrics.error && (
            <Alert variant="destructive">
              <AlertDescription>{(metrics.error as Error).message}</AlertDescription>
            </Alert>
          )}
          {m && (
            <>
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <Gauge label="CPU used" pct={m.total.cpuUsedPct} detail={`${m.total.cpuCores} cores`} />
                <Gauge label="Memory used" pct={m.total.memoryUsedPct} detail={bytes(m.total.memoryBytes)} />
                <Gauge label="CPU reserved" pct={m.total.cpuRequestedPct} detail="by pod requests" tone="sky" />
                <Gauge label="Memory reserved" pct={m.total.memoryRequestedPct} detail="by pod requests" tone="sky" />
              </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Installed by shpyrd cluster init ({c.components.length})</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
                ...
                <TableBody>
                  {helm.data?.map((r) => (
                    <TableRow key={`${r.namespace}/${r.name}`}>
                      ...
                      <TableCell>
                        <Badge variant={r.status === 'deployed' ? 'secondary' : 'destructive'}>{r.status}</Badge>
                      </TableCell>
...
function Info({ label, value, mono, hint }: { label: string; value: string; mono?: boolean; hint?: string }) {
  return (
    <Card size="sm">
      <CardContent>
        <div className="text-xs text-muted-foreground">{label}</div>
        <div className={cn('truncate text-lg font-semibold', mono && 'font-mono text-base')} title={value}>
          {value}
        </div>
...
function Gauge({ label, pct, detail, tone = 'orange' }: { label: string; pct: number; detail?: string; tone?: 'orange' | 'sky' }) {
  ...
  return (
    <div className="grid gap-1.5 rounded-lg border p-3">
      <div className="flex items-baseline justify-between">
        ...
        <span className={cn('font-mono text-sm font-semibold', hot && 'text-red-500')}>{v.toFixed(0)}%</span>
      </div>
...
function Bar({ pct, tone }: { pct: number; tone: 'orange' | 'sky' | 'red' }) {
  return (
    <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
      <div
        className={cn('h-full rounded-full transition-all', tone === 'red' ? 'bg-red-500' : tone === 'sky' ? 'bg-sky-500' : 'bg-primary')}
...
function NodesTable({ nodes, usage }: { nodes: { name: string; ready: boolean; roles: string; arch: string; kubeletVersion: string }[]; usage?: ClusterMetrics }) {
          ...
          return (
            <TableRow key={n.name}>
              <TableCell className="font-medium">
                <span className={cn('mr-2 inline-block size-2 rounded-full', n.ready ? 'bg-emerald-500' : 'bg-red-500')} />
                {n.name}
                <span className="ml-2 font-mono text-[10px] text-muted-foreground">{n.arch}</span>
              </TableCell>
              <TableCell className="text-xs">{n.roles}</TableCell>
              <TableCell>{u ? <UsageCell used={u.cpuUsedPct} reserved={u.cpuRequestedPct} detail={`${u.cpuCores} cores`} /> : '-'}</TableCell>
              <TableCell>{u ? <UsageCell used={u.memoryUsedPct} reserved={u.memoryRequestedPct} detail={bytes(u.memoryBytes)} /> : '-'}</TableCell>
...
function UsageCell({ used, reserved, detail }: { used: number; reserved: number; detail: string }) {
  return (
