...
const styles: Record<string, string> = {
  ...
  Deploying: 'bg-amber-500/15 text-amber-700 dark:text-amber-400 border-amber-500/30',
...
export function PhaseBadge({ phase }: { phase?: string }) {
  ...
  return (
    <Badge variant="outline" className={cn('gap-1.5 font-medium', styles[p] ?? styles.Pending)}>
      <span className={cn('inline-block size-1.5 rounded-full bg-current', live && 'animate-pulse')} />
      {p}
    </Badge>
  )
}
