...
export function SizesEditor({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const save = useMutation({
    ...
    onSuccess: (c) => {
      toast.success(
        "Instance sizes saved; processes using changed sizes are being resized",
      );
  ...
  const invalid =
    draft.sizes.some((s) => !nameRe.test(s.name) || !s.cpu || !s.memory) ||
    !draft.sizes.some((s) => s.name === draft.default);

  const update = (i: number, patch: Partial<InstanceSize>) =>
    setDraft({
      ...draft,
      sizes: draft.sizes.map((s, j) => (j === i ? { ...s, ...patch } : s)),
    });
  ...
  const add = () =>
    setDraft({
      ...
      sizes: [
        ...draft.sizes,
        { name: "", kind: "shared", cpu: "0.5", memory: "128Mi" },
      ],
    });
    ...
    <Card>
      ...
      <CardContent>
              ...
              <TableRow key={i}>
                <TableCell>
                  <input
                    ...
                    disabled={readOnly || !nameRe.test(s.name)}
                ...
                <TableCell>
                  <Input
                    ...
                    disabled={readOnly}
                    className={cn(
                      "h-7 w-36 font-mono text-xs",
                      !nameRe.test(s.name) && "border-red-500",
                    )}
                ...
                <TableCell>
                  <Select
                    value={s.kind}
                    onValueChange={(v) =>
                      update(i, { kind: v as InstanceSize["kind"] })
                    }
                    disabled={readOnly}
                  >
                ...
                <TableCell className="text-right">
                  <Button
                    ...
                    disabled={readOnly || draft.default === s.name}
                    title={
                      draft.default === s.name
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          Processes pick a size in{" "}
          <code className="font-mono">shpyrd.yaml</code> (
          <code className="font-mono">processes.web.size: shared-m</code>), with{" "}
          <code className="font-mono">shpyrd resize web=shared-m</code>, or from
          the project page. Without one they get the default.
        </p>
      </CardContent>
    </Card>
  );
