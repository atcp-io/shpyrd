...
function level(msg: string): Level {
  ...
  if (
    /\b(error|err|fatal|panic|exception|traceback|failed)\b/.test(m) ||
    /"level":"(error|fatal)"/.test(m)
  )
    return "error";
...
export function AppLogView({
  ...
  const visible = useMemo(() => {
    ...
    if (!f) return lines;
    return lines.filter(
      (l) => l.m.toLowerCase().includes(f) || l.i.toLowerCase().includes(f),
    );
  }, [lines, filter]);
        ...
        visible.map((l, idx) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={cn(
                  lvl === "error" && "text-red-200",
                  lvl === "warn" && "text-amber-100",
                )}
              >
                {l.m.replace(ansi, "")}
              </span>
